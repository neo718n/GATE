    ...
    <section id="hangzhou-camp" className="bg-card border-b border-border">
      ...
      <div className="relative h-[60vh] min-h-[480px] w-full overflow-hidden">
        ...
        <div className="relative z-10 h-full flex items-end">
          <div className="mx-auto max-w-7xl w-full px-6 pb-14 flex flex-col gap-5">
            ...
            <h2 className="font-serif text-4xl md:text-6xl font-light text-gate-white leading-[1.1] max-w-3xl">
      ...
      <div className="mx-auto max-w-7xl px-6 py-24 grid grid-cols-1 lg:grid-cols-5 gap-12 items-center border-b border-border">
        ...
        <div className="lg:col-span-3 flex flex-col gap-5">
          ...
          <h3 className="font-serif text-3xl md:text-4xl font-light text-foreground leading-[1.2]">
          ...
          <div className="flex flex-col gap-2 pt-2 text-[12px] font-light text-foreground/55">
            <div className="flex gap-3">
              <span className="text-foreground/35 w-24 shrink-0">Location</span>
              <span>Hangzhou · Qiantang River</span>
            </div>
            <div className="flex gap-3">
              <span className="text-foreground/35 w-24 shrink-0">Original name</span>
              <span>西安电子科技大学杭州研究院</span>
            </div>
            <div className="flex gap-3">
              <span className="text-foreground/35 w-24 shrink-0">Established</span>
          ...
          {PROGRAM_COMPONENTS.map((c) => (
            <article key={c.title} className="flex flex-col bg-card overflow-hidden">
              ...
              <div className="flex flex-col gap-3 p-7">
                ...
                <h4 className="font-serif text-xl font-light text-foreground">{c.title}</h4>
                <p className="text-[12px] font-light text-foreground/55 leading-relaxed">{c.desc}</p>
              </div>
            </article>
          ))}
        </div>
          ...
          {DAILY_SCHEDULE.map((d) => (
            <div key={d.day} className="flex flex-col gap-3 bg-card p-5">
              <div className="flex items-baseline gap-3">
                <span className="font-serif text-2xl font-light text-gate-gold">{d.day.replace("Day ", "")}</span>
                <span className="text-[9px] font-semibold uppercase tracking-[0.3em] text-foreground/40">
                  {d.date}
                </span>
              </div>
              ...
              <ul className="flex flex-col gap-1.5 pt-1">
                {d.items.map((item) => (
                  <li key={item} className="text-[11px] font-light text-foreground/50 leading-snug flex items-start gap-2">
                    <span className="mt-[6px] h-px w-2 shrink-0 bg-gate-gold/40" />
                    {item}
                  </li>
                ))}
              </ul>
          ...
          {[
            ...
            <div key={a.label} className="flex flex-col bg-card overflow-hidden">
              ...
              <div className="flex flex-col gap-1.5 p-5">
                ...
                <span className="text-[11px] font-light text-foreground/55 leading-snug">{a.desc}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-24 grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="flex flex-col gap-5">
          ...
          <ul className="flex flex-col gap-2.5 mt-2">
            {INCLUDES.map((item) => (
              <li key={item} className="flex items-start gap-3 text-[13px] font-light text-foreground/70">
          ...
          <div className="mt-6 pt-6 border-t border-border">
            <span className="text-[10px] font-semibold uppercase tracking-[0.3em] text-foreground/40 block mb-3">
              Not Included
            </span>
            <ul className="flex flex-col gap-1.5">
              {NOT_INCLUDED.map((item) => (
                <li key={item} className="text-[12px] font-light text-foreground/45">— {item}</li>
              ))}
            </ul>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-5 lg:items-end lg:text-right">
          ...
          <h3 className="font-serif text-3xl md:text-4xl font-light text-foreground leading-[1.15]">
            Join the Hangzhou<br />Academic Camp
          </h3>
          <div className="flex flex-col gap-3 mt-3 lg:items-end">
            <div className="flex items-center gap-3 text-[11px] font-light text-foreground/55 uppercase tracking-[0.2em]">
              <span>19–25 July 2026</span>
              <span className="h-3 w-px bg-foreground/20" />
              <span>{round ? dollarsFromCents(round.feeUsd) : "TBA"}</span>
            </div>
            <p className="text-[12px] font-light text-foreground/50 max-w-sm leading-relaxed">
