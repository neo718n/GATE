...
// CIS / Central Asia surfaced first to match GATE's primary audience.
const PRIORITY: CountryInfo[] = [
...
export const COUNTRIES: CountryInfo[] = [...PRIORITY, ...REST];

export const DEFAULT_COUNTRY_ISO = "US";

export function flagUrl(iso2: string, width: 20 | 40 | 80 = 20): string {
  return `https://flagcdn.com/w${width}/${iso2.toLowerCase()}.png`;
}

export function getCountryByIso(iso2: string | null | undefined): CountryInfo {
  const match = iso2 ? COUNTRIES.find((c) => c.iso2 === iso2.toUpperCase()) : undefined;
  return match ?? COUNTRIES.find((c) => c.iso2 === DEFAULT_COUNTRY_ISO)!;
}

export function getCountryByName(name: string | null | undefined): CountryInfo | undefined {
