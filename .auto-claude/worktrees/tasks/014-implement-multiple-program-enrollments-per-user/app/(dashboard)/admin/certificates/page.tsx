    ...
    <div className="flex flex-col gap-8 max-w-5xl">
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-sm font-light text-foreground/60 mt-1">
          {publishedResults.length} results published - certificate issuance managed here.
        </p>
      </div>
      ...
      <div className="border bg-gate-gold/5 border-gate-gold/25 p-5">
        ...
        <p className="text-sm font-light text-foreground/65 leading-[1.9]">
          ...
          managed by the operations team - upload the certificate URL to each result record to
          ...
          {publishedResults.map((r) => (
            ...
            >
              <p className="text-sm font-light text-foreground truncate">
                {r.participant && typeof r.participant === 'object' && 'fullName' in r.participant ? r.participant.fullName : "-"}
              </p>
              <p className="text-xs font-light text-foreground/70">{r.subject && typeof r.subject === 'object' && 'name' in r.subject ? r.subject.name : "-"}</p>
              <p className="text-xs font-light text-foreground/70">{r.cycle && typeof r.cycle === 'object' && 'name' in r.cycle ? r.cycle.name : "-"}</p>
