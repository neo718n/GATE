          ...
          <div className="flex flex-col gap-0">
            {[
              ...
              <div key={item.label} className="flex gap-6 border-b border-gate-fog/70 py-4 last:border-0">
                <span className="text-[10px] font-semibold uppercase tracking-[0.3em] text-gate-gold w-32 shrink-0 pt-0.5">{item.label}</span>
      ...
      <section className="py-24 px-6 bg-gate-fog/40 border-y border-gate-fog/60">
        <div className="mx-auto max-w-7xl flex flex-col gap-12">
          <div className="flex flex-col gap-4">
            <span className="text-[10px] font-semibold uppercase tracking-[0.3em] text-gate-gold">Program Overview</span>
          ...
          <div className="flex flex-col border border-gate-fog">
            {PROGRAM.map((day, i) => (
              <div key={day.day} className={"grid grid-cols-[120px_1fr] gap-6 p-8" + (i < PROGRAM.length - 1 ? " border-b border-gate-fog/60" : "")}>
                <span className="text-[10px] font-semibold uppercase tracking-[0.3em] text-gate-gold pt-0.5">{day.day}</span>
                <div className="flex flex-col gap-2">
                  <h3 className="text-sm font-semibold tracking-[0.05em] text-gate-800">{day.title}</h3>
                  <p className="text-sm font-light text-gate-800/65 leading-[1.9]">{day.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 px-6 bg-gate-white">
        <div className="mx-auto max-w-7xl grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 flex flex-col gap-5">
            <span className="text-[10px] font-semibold uppercase tracking-[0.3em] text-gate-gold">Standards</span>
