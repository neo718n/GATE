...
export const metadata: Metadata = {
  title: "Partnerships",
  description: "Partner with G.A.T.E. Assessment — universities, schools, and academic institutions.",
};
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {BENEFITS.map((b) => (
              <div key={b.title} className="flex flex-col gap-3 border border-gate-fog p-6">
                <h3 className="text-[10px] font-semibold uppercase tracking-[0.3em] text-gate-800">{b.title}</h3>
                <p className="text-sm font-light text-gate-800/65 leading-[1.9]">{b.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-6 bg-gate-fog/40 border-y border-gate-fog/60">
        <div className="mx-auto max-w-7xl flex flex-col gap-12">
          <div className="flex flex-col gap-4">
            <span className="text-[10px] font-semibold uppercase tracking-[0.3em] text-gate-gold">Partnership Types</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {TYPES.map((t) => (
              <div key={t.title} className="flex flex-col gap-3 border border-gate-fog p-6">
                <h3 className="text-[10px] font-semibold uppercase tracking-[0.3em] text-gate-800">{t.title}</h3>
                <p className="text-sm font-light text-gate-800/65 leading-[1.9]">{t.desc}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="flex flex-col gap-0 mt-4">
              {["Submit application","Admin review (5–7 days)","Approval confirmation","Partnership agreement","Onboarding & orientation"].map((step, i) => (
                <div key={step} className="flex items-start gap-4 py-3 border-b border-border/20 last:border-0">
                  <span className="text-[10px] font-bold text-gate-gold w-5 shrink-0">{i + 1}.</span>
                  <span className="text-sm font-light text-gate-800/65">{step}</span>
                </div>
              ))}
            </div>
          </div>
