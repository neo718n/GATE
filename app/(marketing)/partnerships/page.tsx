    ...
    <>
      <section className="py-24 px-6 border-b border-border/30 bg-gate-800/10">
        <div className="mx-auto max-w-7xl flex flex-col gap-4">
          ...
          <h1 className="font-serif text-5xl md:text-6xl font-light text-gate-white">Partnerships</h1>
          <p className="text-base font-light text-gate-white/45 mt-2 max-w-2xl">Partner with G.A.T.E. to extend the Olympiad's reach and bring world-class academic competition to your students and community.</p>
        </div>
      </section>
      ...
      <section className="py-24 px-6">
        <div className="mx-auto max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="flex flex-col gap-6">
            ...
            <h2 className="font-serif text-4xl font-light text-gate-white leading-tight">Join the Global Academic Network</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {BENEFITS.map((b) => (
              <div key={b.title} className="flex flex-col gap-3 border border-border/40 p-6">
                <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-gate-white">{b.title}</h3>
                <p className="text-sm font-light text-gate-white/55 leading-[1.9]">{b.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-6 bg-gate-800/15 border-y border-border/30">
        <div className="mx-auto max-w-7xl flex flex-col gap-12">
          <div className="flex flex-col gap-4">
            ...
            <h2 className="font-serif text-4xl font-light text-gate-white">Who Can Partner</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {TYPES.map((t) => (
              <div key={t.title} className="flex flex-col gap-3 border border-border/40 p-6">
                <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-gate-white">{t.title}</h3>
                <p className="text-sm font-light text-gate-white/55 leading-[1.9]">{t.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-6">
        <div className="mx-auto max-w-7xl grid grid-cols-1 lg:grid-cols-5 gap-16">
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <h2 className="font-serif text-4xl font-light text-gate-white leading-tight">Become a Partner</h2>
            <div className="flex flex-col gap-4 text-sm font-light text-gate-white/60 leading-[1.9]">
            ...
            <div className="flex flex-col gap-0 mt-4">
              {["Submit application","Admin review (5–7 days)","Approval confirmation","Partnership agreement","Onboarding & orientation"].map((step, i) => (
                <div key={step} className="flex items-start gap-4 py-3 border-b border-border/20 last:border-0">
                  ...
                  <span className="text-sm font-light text-gate-white/55">{step}</span>
                </div>
              ))}
            </div>
          </div>
