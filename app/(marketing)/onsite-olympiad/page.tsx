    ...
    <>
      <section className="py-24 px-6 border-b border-border/30 bg-gate-800/10">
        <div className="mx-auto max-w-7xl flex flex-col gap-4">
          ...
          <h1 className="font-serif text-5xl md:text-6xl font-light text-gate-white">Onsite Olympiad</h1>
          <p className="text-base font-light text-gate-white/45 mt-2">Xidian University, Hangzhou Campus, China</p>
        </div>
      </section>
      ...
      <section className="py-24 px-6">
        <div className="mx-auto max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div className="flex flex-col gap-6">
            <h2 className="font-serif text-4xl font-light text-gate-white leading-tight">The Global Academic Final</h2>
          ...
          <div className="flex flex-col gap-0">
            {[
              ...
              <div key={item.label} className="flex gap-6 border-b border-border/25 py-4 last:border-0">
      ...
      <section className="py-24 px-6 bg-gate-800/15 border-y border-border/30">
        <div className="mx-auto max-w-7xl flex flex-col gap-12">
          <div className="flex flex-col gap-4">
            ...
            <h2 className="font-serif text-4xl font-light text-gate-white">7-Day Academic Program</h2>
          </div>
          <div className="flex flex-col border border-border/40">
            {PROGRAM.map((day, i) => (
              <div key={day.day} className={"grid grid-cols-[120px_1fr] gap-6 p-8" + (i < PROGRAM.length - 1 ? " border-b border-border/30" : "")}>
                <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-gate-gold pt-0.5">{day.day}</span>
                <div className="flex flex-col gap-2">
                  <h3 className="text-sm font-semibold tracking-[0.05em] text-gate-white">{day.title}</h3>
                  <p className="text-sm font-light text-gate-white/55 leading-[1.9]">{day.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 px-6">
        <div className="mx-auto max-w-7xl grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 flex flex-col gap-5">
            ...
            <h2 className="font-serif text-4xl font-light text-gate-white">Academic Supervision</h2>
          </div>
          <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {[
              ...
              <div key={item.title} className="flex flex-col gap-3 border border-border/40 p-6">
                <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-gate-white">{item.title}</h3>
                <p className="text-sm font-light text-gate-white/55 leading-[1.9]">{item.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-6 bg-gate-800/15 border-t border-border/30">
        <div className="mx-auto max-w-3xl flex flex-col items-center gap-6 text-center">
          <h2 className="font-serif text-4xl font-light text-gate-white">Qualify for the Global Final</h2>
          <p className="text-sm font-light text-gate-white/55 leading-[1.9]">Begin with the Preliminary Round. Top qualifying participants receive an invitation to the onsite final at Xidian University, Hangzhou.</p>
