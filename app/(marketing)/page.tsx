...
const STATUS_CONFIG: Record<string, { label: string; color: string; dot: string }> = {
  registration_open: { label: "Registration Open", color: "text-emerald-700 dark:text-emerald-400", dot: "bg-emerald-500" },
  active: { label: "Assessment In Progress", color: "text-gate-gold", dot: "bg-gate-gold" },
  planning: { label: "Upcoming", color: "text-foreground/50", dot: "bg-foreground/25" },
  completed: { label: "Completed", color: "text-foreground/40", dot: "bg-foreground/20" },
  archived: { label: "Archived", color: "text-foreground/30", dot: "bg-foreground/15" },
};
...
export default async function HomePage() {
  ...
  return (
    <>
      {/* ANNOUNCEMENT STRIP — gold bg, always dark text for contrast */}
      ...
      {/* HERO */}
      <section className="relative flex min-h-[calc(100vh-56px)] flex-col items-center justify-center px-6 text-center overflow-hidden bg-card">
        ...
        <div className="relative z-10 flex flex-col items-center gap-8 max-w-4xl w-full">
          ...
          <p className="text-base md:text-lg font-light text-foreground/55 leading-relaxed max-w-xl mt-2">
          ...
          <div className="mx-auto max-w-7xl flex flex-col lg:flex-row lg:items-start gap-10">
            <div className="flex-1 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-foreground/35">
                  Current Edition
                </p>
              ...
              {activeCycle.description && (
                <p className="text-sm font-light text-foreground/55 leading-[1.85] max-w-md">
                  {activeCycle.description}
                </p>
              )}
            ...
            {activeCycle.rounds.length > 0 && (
              <div className="lg:w-[420px] flex flex-col border border-border">
                ...
                {activeCycle.rounds.map((round, i) => (
                  <div
                    ...
                    className={`flex items-start gap-5 px-5 py-4 ${
                      i < activeCycle.rounds.length - 1 ? "border-b border-border" : ""
                    }`}
                  >
                    ...
                    <div className="flex flex-col gap-1 flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-sm font-light text-foreground truncate">{round.name}</span>
                      ...
                      {round.venue && (
                        <span className="text-[11px] font-light text-foreground/35">{round.venue}</span>
                      )}
                    </div>
                  </div>
                ))}
      ...
      <section className="py-28 px-6 bg-background border-b border-border">
        <div className="mx-auto max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="flex flex-col gap-5">
            ...
            <h2
              className="font-serif text-4xl md:text-5xl font-light text-foreground leading-[1.2]"
          ...
          <div className="flex flex-col gap-5 text-sm font-light text-foreground/60 leading-[1.9]">
            <p>
      ...
      <section className="py-28 px-6 bg-card border-b border-border">
        <div className="mx-auto max-w-7xl flex flex-col gap-12">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <h2 className="font-serif text-4xl md:text-5xl font-light text-foreground">Six Subject Areas</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-border">
            {SUBJECTS.map((s) => (
              <div
                key={s.code}
                className="flex flex-col gap-3 bg-card p-8 hover:bg-muted/30 transition-colors"
              >
      ...
      <section className="py-28 px-6 bg-background border-b border-border">
        <div className="mx-auto max-w-7xl flex flex-col gap-14">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <h2 className="font-serif text-4xl md:text-5xl font-light text-foreground">Two Rounds. One Path.</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              ...
              <div key={item.tag} className="flex flex-col gap-5 border border-border bg-card p-8">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-light uppercase tracking-[0.18em] text-foreground/55">
                      {item.subtitle}
                    </p>
                  </div>
                </div>
                ...
                <ul className="flex flex-col gap-2.5">
                  {item.points.map((pt) => (
                    <li key={pt} className="flex items-start gap-3 text-sm font-light text-foreground/60">
        ...
        <div className="mx-auto max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="flex flex-col gap-6">
            ...
            <h2 className="font-serif text-4xl md:text-5xl font-light text-foreground leading-tight">
              Onsite Final<br />
              <span className="text-foreground/55">Host Venue Per Edition</span>
            </h2>
            <div className="h-px w-14 bg-gate-gold/40" />
            <p className="text-sm font-light text-foreground/60 leading-[1.9]">
          ...
          <div className="flex flex-col border border-border">
            {[
              ...
              <div
                key={row.label}
                className="flex gap-6 border-b border-border px-6 py-4 last:border-0"
              >
                <span className="text-[10px] font-semibold uppercase tracking-[0.25em] text-gate-gold w-28 shrink-0 pt-0.5">
                  {row.label}
                </span>
                <span className="text-sm font-light text-foreground/65">{row.value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl flex flex-col gap-14">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-border">
            {AWARDS.map((a) => (
              <div
                key={a.tier}
                className="flex flex-col items-center gap-5 bg-card hover:bg-muted/30 transition-colors p-10 text-center"
              >
                <div
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="text-[11px] font-semibold tracking-[0.05em] text-foreground">{a.tier}</span>
                  <span className="text-[10px] font-light text-foreground/50 leading-relaxed">{a.desc}</span>
                </div>
              </div>
            ))}
          </div>
          <p className="text-center text-xs font-light text-foreground/45">
      ...
      {/* STATS — intentional dark band, same in both light and dark modes */}
      ...
      <section className="py-28 px-6 bg-card border-b border-border">
        <div className="mx-auto max-w-4xl flex flex-col gap-12">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <h2 className="font-serif text-4xl font-light text-foreground">Frequently Asked Questions</h2>
          </div>
      ...
      {/* CTA — intentional dark section, same in both light and dark modes */}
